...
# Category: Different Probability in Different Directions
# ----------------------------- Basic Info ------------------------------
# Lattice site:                 Square site
# Dimensions:                   2
# Initial cluster:              A point at the origin
# Prob. to move at directions:  P_left = 0.1, others = 0.3
    ...
    while(True):
        ...
        if(temp == 1):
            if(curDist > R_max*R_max):
                ...
                return 1
        elif(curDist > (min(max(1.5*R_current_max, R_current_max + delta), latticeRange-1)) * (min(max(1.5*R_current_max, R_current_max + delta), latticeRange-1))):
            ...
            if(rand == 0):
                ...
                lattice[x+1][y] = progress
                x, y = x+1, y
            elif(rand == 2 or rand == 3 or rand == 4 or rand == 5):
                lattice[x][y] = BackgroundColorPara
