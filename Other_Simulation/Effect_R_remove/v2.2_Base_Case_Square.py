...
# Category: Base Case with Modified R_remove
# ----------------------------- Basic Info ------------------------------
...
step = 1
# Remove radius (w.r.t. multiple of R_current_max)
multiple = 1.4
    ...
    while(True):
        ...
        if(temp == 1):
            if(curDist > R_max*R_max):
                ...
                return 1
        elif(curDist > (min(max(multiple*R_current_max, R_current_max + delta), latticeRange-1)) * (min(max(multiple*R_current_max, R_current_max + delta), latticeRange-1))):
...
def plotFigure(SuccessfullyClusteringRate):
    ...
    plotname = f'Clustering_rate={"{:.2f}%".format(SuccessfullyClusteringRate*100)}_Remove={multiple}.png'
