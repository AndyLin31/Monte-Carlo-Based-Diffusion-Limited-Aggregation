...
# Category: Modified Boundary Condition
# ----------------------------- Basic Info ------------------------------
...
def setInitialCluster():
    ...
    # Set the round boundary radius
    global boundaryRadius
    boundaryRadius = 120
    for i in range(10000):
        x = int(latticeSide//2 + boundaryRadius*np.cos(i/1000*2*np.pi))
        y = int(latticeSide//2 + boundaryRadius*np.sin(i/1000*2*np.pi))
        lattice[x][y] = float(1)/numOfParticles
    
    lattice[latticeSide//2][latticeSide//2] = float(1)/numOfParticles
...
def randGenerate(lattice, radius, progress):
    temp = 0
    while(not temp):
        actual_radius = boundaryRadius*np.random.random()
        theta = 2*np.pi*np.random.random()
        x = int(latticeSide//2 + actual_radius*np.cos(theta))
        y = int(latticeSide//2 + actual_radius*np.sin(theta))
        if(lattice[x][y] < 0):
...
def sticksToCluster(x, y):
    if(lattice[x-1][y] > 0 or lattice[x+1][y] > 0 or lattice[x][y-1] > 0 or lattice[x][y+1] > 0):
        return 1
    return 0
    ...
    while(True):
        ...
        if(temp == 1):
            if(curDist > R_max*R_max):
                ...
                global R_current_max
                if(math.sqrt(curDist) < 0.7 * boundaryRadius):
            ...
            # print('curDist', math.sqrt(curDist))
            return 0
        else:
...
def main():
    ...
    while count <= numOfParticles:
        ...
        # print("R_current_max:", R_current_max, "delta:", delta)
