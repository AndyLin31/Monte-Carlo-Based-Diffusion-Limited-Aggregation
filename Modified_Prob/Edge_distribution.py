...
convolve_num = 10
log_radius = np.log(np.arange(10, max_radius-(convolve_num+138)))

# Define the convolution window
window = np.ones(convolve_num) / convolve_num

edge_distribution = [density_distribution[i+1]-density_distribution[i] for i in range(10, max_radius-(convolve_num+129))]
