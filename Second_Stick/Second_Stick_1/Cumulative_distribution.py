...
data = np.empty((file_number, 1940, 1940))
...
density_distribution = [points_within_radius(all_distances, r) for r in range(max_radius+1-50)]
...
iavg += np.log(file_number)
linear_regression = avg * log_radius + iavg
...
# Add formula to plot
formula_text = f'y = {avg:.4f} * x + {iavg:.4f}'
plt.text(0.30, 0.80, formula_text, transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
